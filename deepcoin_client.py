...
架构基底: 基于 Deepcoin V1/V2 (类 OKX V5 架构) 高级 REST API 构建
"""
...
class DeepcoinClient:
    ...
    def get_position_info(self, symbol="ETH-USDT-SWAP"):
        """获取持仓明细"""
        res = self._request("GET", "/deepcoin/account/positions", {"instType": "SWAP", "instId": symbol})
        return res
    ...
    def place_limit_order(self, symbol, side, price, amount, leverage=20):
        """
        三段递进狙击：下达限价单 (Limit Order)
        注意：Deepcoin 的 size (sz) 通常为张数 (Cont)，系统大脑需计算好张数传入
        """
        ...
        params = {
            "instId": symbol,
            "tdMode": "cross",         # 全仓模式
            "side": ord_side,          # 开多为 buy, 开空为 sell
            "ordType": "limit",        # 限价单
            "sz": str(int(amount)),    # 深币要求张数为整数形式的字符串
            "px": str(round(price, 2)),
            "posSide": pos_side,       # 明确持仓方向
            "mrgPosition": "merge"     # 合仓模式，绝不锁仓
        }
    ...
    def cancel_all_open_orders(self, symbol="ETH-USDT-SWAP"):
        ...
        # 深币提供的极速一键撤单接口
        params = {
            "InstrumentID": symbol.replace("-SWAP", "").replace("-", ""), # 例如 BTCUSDT
            "ProductGroup": "SwapU",
            "IsCrossMargin": 1,
            "IsMergeMode": 1
        }
        res = self._request("POST", "/deepcoin/trade/swap/cancel-all", params)
        return res
    ...
    def close_all_positions(self, symbol="ETH-USDT-SWAP"):
        ...
        res = self._request("POST", "/deepcoin/trade/batch-close-position", params)
        return res

    def close_position_partial(self, symbol="ETH-USDT-SWAP", action="LONG", close_ratio=0.5):
        """
        精确切割：市价平掉 50% 的仓位 (实现 7U 落袋)
        """
        # 1. 必须先查当前持有多少张
        ...
        try:
            ...
            params = {
                "instId": symbol,
                "tdMode": "cross",
                "side": ord_side,          # 平多用 sell，平空用 buy
                "ordType": "market",       # 刺客市价抢跑
                "sz": str(close_sz),
                "posSide": pos_side,       # 必须指明是在操作 long 还是 short
                "mrgPosition": "merge"
            }
            return self._request("POST", "/deepcoin/trade/order", params)
